...
import type { Cache, CollectionSchema, RecordRow } from '@ingitdb/client'
import { buildCacheKey, parseCollectionSchema, parseYaml } from '@ingitdb/client'
...
export const loadCollectionRecords = async (
  ...
  if (isNested) {
    const dirs = entries.filter((f) => f.type === 'dir' && !f.name.startsWith('.')).slice(0, deps.maxRecords ?? DEFAULT_MAX_RECORDS)
    ...
    filesToFetch = entries
      ...
      .slice(0, deps.maxRecords ?? DEFAULT_MAX_RECORDS)
