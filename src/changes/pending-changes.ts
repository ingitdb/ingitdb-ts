...
export function createPendingChangesStore(githubApi: GithubApi, cache: Cache): PendingChangesStore {
  ...
  const clearAll = async (userId: string, repo: string, branch: string): Promise<void> => {
    const db = await dbPromise
    const changes = await db.getAllFromIndex(STORE_NAME, 'by-repo-branch', [userId, repo, branch])
    for (const change of changes as PendingChange[]) {
      await db.delete(STORE_NAME, [change.userId, change.repo, change.branch, change.collectionId, change.recordId])
    }
  }
