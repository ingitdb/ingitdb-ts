...
import type { CommittedChangesStore } from './committed-changes'
...
describe('PendingChangesStore', () => {
  ...
  describe('commitAll', () => {
    ...
    it('skips non-delete changes (covers changeType !== delete → continue and changeByPath.size === 0 → continue)', async () => {
      // The public API only has stageDelete, so we seed a non-delete row directly
      // into the shared IDB rows Map to reach the `continue` branches at lines 123 and 143.
      ...
      store = createPendingChangesStore(githubApi, mockCache())
      await store.commitAll(commitParams)
      // Non-delete → changeByPath stays empty → group is skipped → no git API calls
      expect(githubApi.getBranchSHA).not.toHaveBeenCalled()
    })
